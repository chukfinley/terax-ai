...
- **CLI agent providers** (`cli/`): wrap locally-installed coding-agent CLIs (Claude Code, Codex, cursor-agent, OpenCode) as keyless providers (`cli-claude`/`cli-codex`/`cli-cursor`/`cli-opencode` in `config.ts`). They are full agents, not raw models: `runAgentStream` branches on `isCliProvider` to `runCliAgentStream`, which skips Terax's tools/compaction and hands the CLI a flattened transcript. The Rust side (`agent_cli.rs`: `agent_cli_which`/`agent_cli_spawn`/`agent_cli_kill`) is a generic, injection-safe spawner: argv passed as separate args (never a shell string), binary resolved against the user's login PATH (GUI apps inherit a minimal PATH), cwd gated through `authorize_spawn_cwd`, stdout/stderr lines relayed verbatim over a `Channel`. Per-CLI JSON event parsing lives entirely in `cli/parsers/*` (one stateful parser each), mapping to AI SDK `UIMessageChunk`s via `ChunkEmitter` so adding a CLI never touches Rust. Permission posture (plan/auto-edit/full) is the `cliAgentPermission` preference, read at spawn time and mapped to each CLI's native flags.
